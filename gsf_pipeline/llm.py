...
def groq_analyse_rss(titre: str, contenu: str) -> dict:
    ...
    prompt = (
        ...
        "- Événements climatiques étrangers (hors UE) sans lien réglementaire FR/UE\n"
        "- Catastrophes naturelles à l'étranger, conflits, faits divers, sport, défense\n"
        "- Biodiversité/nature sans lien avec obligations légales entreprises\n"
        "RÈGLE STRICTE : si le lien avec GSF nécessite plus d'une déduction → NON PERTINENT.\n\n"
        "Score (exigeant — score 2/3 réservé aux signaux à fort impact concret) :\n"
        "  1 = tendance générale à connaître, pas d'implication directe immédiate pour GSF\n"
        "  2 = évolution réglementaire ou marché qui impose une action identifiable à GSF\n"
        "  3 = obligation légale imminente, risque direct ou opportunité stratégique claire\n"
        "PAR DÉFAUT score=1. Score 2 uniquement si GSF doit adapter une pratique ou un reporting "
        "dans les 12 mois. Score 3 uniquement si GSF est directement visé par une obligation "
        "ou un risque opérationnel concret.\n\n"
        f"TITRE: {titre}\n"
